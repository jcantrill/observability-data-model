...
| `log_source` | resource | all | stream label | Compatibility attribute, contains same information as `openshift.log.source` |
| `openshift.log.type` | resource | all | stream label | |
| `log_type` | resource | all | stream label | Compatibility attribute, contains same information as `openshift.log.type` |
...
| `k8s.event.level` | log | audit | structured metadata | |
| `k8s.event.stage` | log | audit | structured metadata | |
| `k8s.event.user_agent` | log | audit | structured metadata | |
| `k8s.event.request.uri` | log | audit | structured metadata | |
| `k8s.event.response.code` | log | audit | structured metadata | |
| `k8s.event.annotations.*` | log | audit | structured metadata | |
| `k8s.event.object_ref.resource` | log | audit | structured metadata | |
| `k8s.event.object_ref.name` | log | audit | structured metadata | |
| `k8s.event.object_ref.api.group` | log | audit | structured metadata | |
| `k8s.event.object_ref.api.version` | log | audit | structured metadata | |
